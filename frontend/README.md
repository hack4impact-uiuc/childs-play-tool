...
#### Syntax Highlighting
We recommend you use Naomi for syntax highlighting. Install `Naomi` from Package Control and set all javascript files to use it for syntax highlighting.
...
For Flow:
1) Install `Flow` via package control
2) Make sure `flow` is enabled by going into package control -> `SublimeLinter: Toggle Linter` -> `flow` -> `enabled`
